 ...
 * @author jin song
 */
public class EnumOpTypeHandler extends BaseTypeHandler<OpType> {

    public EnumOpTypeHandler() {
    }
     ...
     */
    private OpType locateEnum(int enumVal) {
        for (OpType opType : OpType.class.getEnumConstants()) {
            if (opType.getIntVal() == enumVal) {
                return opType;
            }
        }
        throw new IllegalArgumentException("错误的OpType枚举类型：" + enumVal + ",请核对" + OpType.class.getSimpleName());
    }
}
